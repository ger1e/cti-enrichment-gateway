...
def _attr_specs(attributes: dict, provider: str, result: dict) -> list[EntitySpec]:
    ...
    for key, kind, label in candidates:
        ...
        specs.append(EntitySpec(_entity_type(kind, value), value, label, _note(result, provider), {'para11ax.provider': provider}))
    return specs
...
def _evidence_spec(item: dict, result: dict) -> EntitySpec:
    ...
    properties = {'para11ax.provider': provider, 'para11ax.verdict': verdict, 'para11ax.kind': kind}
    ...
    if parser:
        properties['para11ax.parser_version'] = parser
    if fingerprint:
        properties['para11ax.fingerprint'] = fingerprint[:64]
    if cache_state:
        properties['para11ax.cache_state'] = cache_state
    if retrieved:
        properties['para11ax.retrieved_at'] = retrieved
    duration = item.get('durationMs')
    if isinstance(duration, (int, float)) and duration >= 0:
        properties['para11ax.duration_ms'] = str(duration)
...
def _correlation_specs(result: dict) -> list[EntitySpec]:
    ...
    for item in correlation.get('corroboration') or []:
        ...
        specs.append(EntitySpec('maltego.Phrase', f'Corroboration: {semantic} / {polarity} ({len(providers)} providers)', 'corroboration', _note(result), {'para11ax.semantic_class': semantic, 'para11ax.polarity': polarity, 'para11ax.providers': ','.join(providers)}))
    for item in correlation.get('contradictions') or []:
        ...
        specs.append(EntitySpec('maltego.Phrase', f'Contradiction: {semantic}', 'contradiction', _note(result), {'para11ax.semantic_class': semantic, 'para11ax.positive_providers': positive, 'para11ax.negative_providers': negative}))
    ...
    if overall:
        specs.append(EntitySpec('maltego.Phrase', f'Freshness: {overall}', 'freshness', _note(result), {'para11ax.freshness': overall}))
    ...
    if level:
        properties = {'para11ax.huntability': level}
        reason = _string(huntability.get('reason'))
        if reason:
            properties['para11ax.reason'] = reason
    ...
    if kev:
        ...
        specs.append(EntitySpec('maltego.Phrase', f'KEV: {state}{suffix}', 'CVE risk', _note(result), {'para11ax.risk_axis': 'kev'}))
    ...
    if epss and isinstance(epss.get('score'), (int, float)):
        ...
        specs.append(EntitySpec('maltego.Phrase', f"EPSS: {epss['score']}{suffix}", 'CVE risk', _note(result), {'para11ax.risk_axis': 'epss'}))
    cvss = axes.get('cvss') if isinstance(axes.get('cvss'), dict) else None
    if cvss and isinstance(cvss.get('score'), (int, float)):
        specs.append(EntitySpec('maltego.Phrase', f"CVSS: {cvss['score']}", 'CVE risk', _note(result), {'para11ax.risk_axis': 'cvss'}))
    return specs
...
def map_enrichment(result: dict, max_entities: int = 50, include_provider_nodes: bool = False) -> list[EntitySpec]:
    ...
    for rel in result.get('relationships') or []:
        ...
        specs.append(EntitySpec(_entity_type(kind, value), value, relation[:80], _note(result, provider), {'para11ax.provider': provider, 'para11ax.relationship': relation}))
    ...
    for family in hunt.get('families') or []:
        ...
        if value:
            specs.append(EntitySpec('maltego.Phrase', value, 'malware family', _note(result), {'para11ax.kind': 'malware_family'}))
    for actor in hunt.get('actors') or []:
        value = _string(actor)
        if value:
            specs.append(EntitySpec('maltego.Phrase', value, 'actor', _note(result), {'para11ax.kind': 'actor'}))
    ...
    for item in result.get('evidence') or []:
        ...
        if family:
            specs.append(EntitySpec('maltego.Phrase', family, 'malware family', _note(result, provider), {'para11ax.provider': provider}))
        if actor:
            specs.append(EntitySpec('maltego.Phrase', actor, 'actor', _note(result, provider), {'para11ax.provider': provider}))
        integrity = item.get('integrity') if isinstance(item.get('integrity'), dict) else {}
