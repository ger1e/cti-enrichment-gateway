...
export const virustotalProvider = Object.freeze({
  name: 'virustotal', types: ['ip', 'domain', 'url', 'hash'], requiredEnv: 'VIRUSTOTAL_API_KEY', cacheTtlMs: 21600000, negativeCacheTtlMs: 3600000, costClass: 'scarce', timeoutMs: 7000, parserVersion: 'v3-2026-08-20',
  coverageObservationTypesByType: COVERAGE_OBSERVATION_TYPES,
  async run(input, context = {}) {
    const key = requireEnv(context, 'VIRUSTOTAL_API_KEY'); const raw = await fetchJson(`https://www.virustotal.com/api/v3/${vtPath(input)}`, { ...context, headers: { 'x-apikey': key }, maxBytes: 3_000_000 });
    const a = raw?.data?.attributes ?? {}; const stats = a.last_analysis_stats ?? {};
    return { observationType: 'multi_engine_reputation', verdict: vtVerdict(stats), lastSeen: isoFromUnix(a.last_analysis_date), tags: compact(a.tags), attributes: { reputation: a.reputation ?? null, lastAnalysisStats: stats, typeDescription: a.type_description ?? null, meaningfulName: a.meaningful_name ?? null }, relationships: [], references: ['https://www.virustotal.com/gui/home/search'] };
  },
});
