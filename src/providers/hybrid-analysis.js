...
export const hybridAnalysisProvider = Object.freeze({
  ...
  async run(input, context = {}) {
    const key = requireEnv(context, 'HYBRID_ANALYSIS_API_KEY'); const raw = await fetchJson(`https://hybrid-analysis.com/api/v2/search/hash?hash=${encodeURIComponent(input.value)}`, { ...context, headers: { 'api-key': key, 'User-Agent': 'Falcon Sandbox' }, maxBytes: 3_000_000 });
