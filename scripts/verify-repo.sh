...
finalizer_ok() {
  ...
  [[ -s "${script}" ]] && grep -Fq 'branches/main/protection' "${script}" && grep -Fq 'Tooling smoke' "${script}" && grep -Fq 'required_pull_request_reviews' "${script}" && grep -Fq 'required_status_checks' "${script}" && grep -Fq 'enforce_admins' "${script}" && grep -Fq 'allow_force_pushes' "${script}" && grep -Fq 'allow_deletions' "${script}" && grep -Fq 'bootstrap-vercel.ps1' "${script}" && grep -Fq "'scripts/finalize.ps1'" "${workflow}"
}
