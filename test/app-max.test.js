...
test('gateway skips credentialed adapters with missing secrets but keeps no-key IP adapters', async () => {
  ...
  const app = createApp({ env: baseEnv, fetchImpl, cache: new TtlCache() }); const out = await app.handleEnrich(req('8.8.8.8'));
...
test('configured provider is activated in deterministic IP workflow position', async () => {
  ...
  const app = createApp({ env, fetchImpl, cache: new TtlCache() }); const out = await app.handleEnrich(req('8.8.8.8'));
