...
test('IP enrichment uses active RDAP workflow', async () => {
  const seen = [];
  const fetchImpl = async (url) => {
    seen.push(String(url));
    return new Response(JSON.stringify({ handle: 'NET-X', country: 'US' }), { status: 200 });
  };
  const app = createApp({ env, fetchImpl, cache: new TtlCache(), now: () => '2026-08-20T12:00:00Z' });
...
test('CVE enrichment runs KEV then EPSS and preserves separate evidence semantics', async () => {
  const seen = [];
  const fetchImpl = async (url) => {
    const u = String(url); seen.push(u);
    if (u.includes('known_exploited_vulnerabilities.json')) {
      return new Response(JSON.stringify({ vulnerabilities: [{ cveID: 'CVE-2026-12345', product: 'Thing', dateAdded: '2026-08-20' }] }), { status: 200 });
    }
    if (u.startsWith('https://api.first.org/')) {
      return new Response(JSON.stringify({ data: [{ cve: 'CVE-2026-12345', epss: '0.7', percentile: '0.95', date: '2026-08-20' }] }), { status: 200 });
    }
