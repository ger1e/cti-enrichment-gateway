...
const base = {
  ...
  egressClass: 'none', sideEffect: 'none', capabilities: [], usage: 'source', summary: 'fixture source',
};

const registry = createCommandRegistry([
  { ...base, id: 'fixture.source', tokens: ['source'], aliases: [], handler: 'source' },
...
function executor(calls, overrides = {}) {
  return {
    async execute({ descriptor, args, input, signal }) {
      ...
      if (descriptor.handler === 'source') return { type: 'records', value: [{ id: 1 }, { id: 2 }, { id: 3 }] };
...
test('runtime stops after the first failing stage', async () => {
  ...
  assert.deepEqual(calls.map(call => call.handler), ['source']);
});
...
test('abort and unexpected upstream failures are normalized without leaking executor text', async () => {
  ...
  aborted.abort();
  await assert.rejects(
    () => executePipeline({ type: 'pipeline', stages: [{ type: 'invocation', tokens: ['source'] }] }, { registry, executor: executor([]), context: { surface: 'web', authenticated: true, capabilities: [] }, signal: aborted.signal }),
    error => error.code === 'OPERATION_ABORTED',
  );

  const secret = 'TOPSECRET-UPSTREAM';
  await assert.rejects(
    () => executePipeline({ type: 'pipeline', stages: [{ type: 'invocation', tokens: ['source'] }] }, {
      registry,
      executor: executor([], { source: async () => { throw new Error(secret); } }),
...
test('runtime enforces output bounds after executor execution', async () => {
  ...
  await assert.rejects(
    () => executePipeline({ type: 'pipeline', stages: [{ type: 'invocation', tokens: ['source'] }] }, {
      registry,
      executor: executor(calls, { source: async () => ({ type: 'records', value: [{}, {}] }) }),
      context: { surface: 'web', authenticated: true, capabilities: [] },
      signal: new AbortController().signal,
