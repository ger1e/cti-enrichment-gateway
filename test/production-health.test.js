...
test('bootstrap minimizes plaintext gateway bearer lifetime and reacquires it from DPAPI for health only', () => {
  const setToken = bootstrap.indexOf("Set-SensitiveVercelEnv -Vercel $Vercel -Name 'CTI_GATEWAY_TOKEN' -Value $gatewayToken");
  const providerLoop = bootstrap.indexOf("foreach ($name in $SecretNames | Where-Object { $_ -ne 'CTI_GATEWAY_TOKEN' })", setToken);
