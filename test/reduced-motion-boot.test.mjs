...
import { createUnixBootSequence, UNIX_BOOT_LINES } from '../app/boot.js';

test('reduced motion preserves Unix boot, modem, Pepe, services and shell readiness', async () => {
  const stages = [];
  const cues = [];
  const sleeps = [];
  const boot = createUnixBootSequence({
    reducedMotion: true,
  ...
  assert.equal(stages.filter(([name]) => name === 'boot-line').length, UNIX_BOOT_LINES.length);
