...
test('slow globe rotation remains available while aggressive glitches stay reduced', async () => {
...
test('red globe scanner is persistent and uses the canonical alert channel', async () => {
  const polishJs = await read('app/terminal-polish.js');
  const polishCss = await read('app/shell-polish.css');
  assert.match(polishJs, /boot-globe-scanner/);
  assert.match(polishCss, /@keyframes\s+globe-red-scan/);
  assert.match(polishCss, /\.boot-globe-scanner\{[^}]*stroke:var\(--terminal-alert\)[^}]*stroke-opacity:1[^}]*animation:globe-red-scan/);
